...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$product_id = $this->db->getLastId();
				...
				$this->db->query(" INSERT INTO " . DB_PREFIX . "product_to_seller SET seller_id = '" . (int)$data['seller_id'] . "', product_id = '" . (int)$product_id . "'");
			}
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['image'])) {
			...
			if (isset($data['seller_id'])) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_seller WHERE product_id = '" . (int)$product_id . "'");
				$this->db->query(" INSERT INTO " . DB_PREFIX . "product_to_seller SET seller_id = '" . (int)$data['seller_id'] . "', product_id = '" . (int)$product_id . "'");
			}
	...
	public function getProduct($product_id) {
			...
			$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_seller ps ON (p.product_id = ps.product_id) LEFT JOIN " . DB_PREFIX . "sellers s ON (ps.seller_id = s.seller_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getProducts($data = array()) {
			...
			$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_seller ps ON (p.product_id = ps.product_id) LEFT JOIN " . DB_PREFIX . "sellers s ON (ps.seller_id = s.seller_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (isset($data['filter_quantity']) && $data['filter_quantity'] !== '') {
			...
			if (isset($data['filter_seller']) && $data['filter_seller'] !== '') {
				$sql .= " AND ps.seller_id = '" . (int)$data['filter_seller'] . "'";
			}
